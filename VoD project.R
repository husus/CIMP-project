...
# u_occupation <- sample(c(1,2,3,4,5),num_users,replace=T, )
...
USERS <- data.frame(u_id, u_gender, u_age, u_weekly_utilisation, u_sub_utilisation, u_format_pref,
...
# to add error from rnorm

# score = u_genre_pref*80 + u_format_pref*100 + u_age(1|2)*30 - u_age(3|4|5|6)*30 
...
USERS$score2 <- scale(USERS$score)

USERS$treated <- sample(0:1,num_users,replace=T)


USERS[,churn:=ifelse(score2>0,0,1)]
# to do: switch for some ids 0 and 1
