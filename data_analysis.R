...
setwd("C:\\Users\\tommy\\Desktop\\CIMP\\CIMP-project")
source('C:/Users/tommy/Desktop/CIMP/CIMP-project/functions_group4.R')

## 1.0 Data Preparation ##

# Import dataset
finale <- read.csv("C:\\Users\\tommy\\Desktop\\CIMP\\CIMP-project\\full_data_logit.csv")
df_comparison <-read.csv("C:\\Users\\tommy\\Desktop\\CIMP\\CIMP-project\\comparison_data.csv")
  ...
  for(j in seq(1, length(vars))){
    ...
    pgr=ggplot(data.m, aes(category, value)) + geom_bar(aes(fill = variable), 
      ...
      labs(title=sprintf("Uplift Across %s Categories", vars[j]), 
           y="Perc. of  Resub", x=sprintf("%s Categories", vars[j]) )+mytheme+scale_fill_manual(values = c("#E50914", "#000000", "#564d4d"))
